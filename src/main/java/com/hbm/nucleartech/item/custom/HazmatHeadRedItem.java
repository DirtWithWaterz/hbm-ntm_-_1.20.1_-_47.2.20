...
public class HazmatHeadRedItem extends GasmaskItem {

	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

	public HazmatHeadRedItem(Properties s, ArmorMaterial material) {
		super(
				s,
				material,
				"hbm:textures/item/armor/hazmat_red.png",
				null,
				"animation.hazmat_red.no_filter",
				"animation.hazmat_red.filter"
		);
	}
		...
		for(ArmorItem item : set) {
			...
			switch (item.getType()) {
				...
				case HELMET -> {
					...
					if(!item.getDefaultInstance().is(RegisterItems.HAZMAT_HELMET_RED.get()))
						return false;
				}
				case CHESTPLATE -> {

					if(!item.getDefaultInstance().is(RegisterItems.HAZMAT_CHESTPLATE_RED.get()))
						return false;
				}
				case LEGGINGS -> {

					if(!item.getDefaultInstance().is(RegisterItems.HAZMAT_LEGGINGS_RED.get()))
						return false;
				}
				case BOOTS -> {

					if(!item.getDefaultInstance().is(RegisterItems.HAZMAT_BOOTS_RED.get()))
						return false;
				}
			}
		}
